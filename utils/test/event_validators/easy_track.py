...
def validate_evmscript_factory_added_event(event, p: EVMScriptFactoryAdded):
...
def validate_evmscript_factory_removed_event(event, factory_addr: str):
...
def validate_motions_count_limit_changed_event(event, motions_count_limit: int):
