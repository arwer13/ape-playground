...
def validate_set_version_event(event, version: int):
...
def validate_set_el_rewards_vault_event(event, address: str):
...
def validate_set_el_rewards_vault_withdrawal_limit_event(event, limit_points: int):
...
def validate_staking_resumed_event(event):
...
def validate_staking_limit_set(event, max_staking_limit: int, stake_limit_increase: int):
...
def validate_set_fee_distribution(event, treasury_bp: int, insurance_bp: int, operators_bp: int):
