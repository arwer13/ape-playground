...
def validate_token_payout_event(event, p: Payout, is_steth: bool = False):
...
def validate_ether_payout_event(event, p: Payout):
...
def validate_agent_execute_ether_wrap_event(event, p: Payout):
