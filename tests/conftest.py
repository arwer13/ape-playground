...
def ldo_holder(accounts):
    return accounts[LDO_HOLDER_ADDRESS_FOR_TESTS]
...
class Helpers:
    ...
    @staticmethod
    def execute_vote(accounts, vote_id, dao_voting, topup=int(0.1*1e18), skip_time=MAINNET_VOTE_DURATION):
        (tx,) = Helpers.execute_votes(accounts, [vote_id], dao_voting, topup, skip_time)
        return tx

    @staticmethod
    def execute_votes(accounts, vote_ids, dao_voting, topup=int(0.1*1e18), skip_time=MAINNET_VOTE_DURATION):
        ...
        for vote_id in vote_ids:
            ...
            if dao_voting.canVote(vote_id, LDO_VOTE_EXECUTORS_FOR_TESTS[0]) and (
                ...
                for holder_addr in LDO_VOTE_EXECUTORS_FOR_TESTS:
                    if Address(holder_addr).balance < topup:
                        accounts[ETH_WHALE].transfer(holder_addr, topup)
                    dao_voting.vote(vote_id, True, False, sender=holder_addr)

        # wait for the vote to end
        # chain.sleep(skip_time)
        chain.mine(deltatime=skip_time)
        chain.mine()
        ...
        try:
            _ = project.CallsScript.at(executor_addr)
        except:
        ...
        for vote_id in vote_ids:
            tx = dao_voting.executeVote(vote_id, sender=ETH_WHALE)
