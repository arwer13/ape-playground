...
def test_vote(
    ...
    withdrawal_vault_manager = project.WithdrawalVaultManager.at(WITHDRAWAL_VAULT)
    ...
    lido_proxy = project.AppProxyUpgradeable.at(contracts.lido)
    ...
    nor_proxy = project.AppProxyUpgradeable.at(contracts.node_operators_registry)
    ...
    oracle_proxy = project.AppProxyUpgradeable.at(contracts.legacy_oracle)
...
def validate_set_base_uri_event(events, base_uri: str):
...
def validate_withdrawal_vault_manager_upgrade_events(events, implementation: str):
...
def validate_start_upgrade_events(events):
...
def validate_finish_upgrade_events(events):
